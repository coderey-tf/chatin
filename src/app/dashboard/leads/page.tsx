...
export default async function ClientLeadsRedirectPage() {
  const customers = await listCustomers()
  if (customers.length > 0) {
    redirect(`/dashboard/customers/${customers[0].id}/leads`)
  }
    ...
    <div className="min-h-[50vh] flex flex-col items-center justify-center text-center p-6">
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-8 max-w-md w-full">
        ...
        <p className="text-zinc-400 text-xs mb-6">
          Silakan jalankan script SQL Migration Supabase di SQL Editor terlebih dahulu atau hubungkan akun WhatsApp Business API.
        </p>
        <Link
