...
const NAME_PATTERNS = [
  ...
  // 2. atas nama / a.n / a/n (stops at periods, commas, colons or before words like tgl/tanggal/untuk/di)
  /(?:atas\s+nama|a[./]n)\s*:?\s*([A-Za-z'-]{2,25}(?:\s+[A-Za-z'-]{2,25}){0,2})/i,

  // 3. nama saya / namaku / saya / aku
  /(?:nama\s*(?:saya|aku)?|namaku)\s*:?\s+([A-Za-z'-]{2,25}(?:\s+[A-Za-z'-]{2,25}){0,2})/i,
 ...
 */
function extractExplicitFormLine(
  text: string,
  field: BotField,
): string | undefined {
...
function isFuzzyMatch(textWord: string, targetKeyword: string): boolean {
  ...
  if (
    GREETING_OR_CONVERSATIONAL_WORDS.includes(w) &&
    !GREETING_OR_CONVERSATIONAL_WORDS.includes(k)
  ) {
    return false;
  }
    ...
    case "text": {
      ...
      if (field.key === "name" || field.key === "contact_name") {
        ...
        for (const line of lines) {
          ...
          if (/\d/.test(line) || line.includes(":") || line.length > 35)
            continue;

          // Skip if line contains greetings
          if (GREETING_WORDS.some((gw) => line.toLowerCase().includes(gw)))
            continue;
          ...
          if (LOCATIONS.some((loc) => line.toLowerCase().includes(loc)))
            continue;
...
export function isGreeting(text: string, fields: BotField[]): boolean {
  ...
  for (const field of fields) {
    ...
    if (field.type === "date" && /\d/.test(text) && lower.length > 10)
      return false;
    if (
      (field.key === "name" || field.key === "contact_name") &&
      (NAME_PATTERNS.some((p) => p.test(text)) ||
        extractExplicitFormLine(text, field))
    )
      return false;
  }
  return true;
...
export function accumulateLeadData(
  ...
  for (const [k, v] of Object.entries(current)) {
    if (v) {
      // If name is already established from history, only overwrite if current message explicitly has "nama" label
      if ((k === "name" || k === "contact_name") && accumulated[k]) {
        const nameField = fields.find((f) => f.key === k);
        const hasExplicitName = nameField ? extractExplicitFormLine(currentMessage, nameField) : null;
        const hasPatternName = NAME_PATTERNS.some((p) => p.test(currentMessage));
        if (!hasExplicitName && !hasPatternName) {
          continue;
        }
      }
      accumulated[k] = v;
  ...
  for (const field of fields) {
    if (!accumulated[field.key] && field.default_value) {
      const nameKey = fields.find(
        (f) => f.key === "name" || f.key === "contact_name",
      )?.key;
      const hasName = nameKey ? !!accumulated[nameKey] : true;
      const requiredKeys = fields
        .filter((f) => f.required && f.key !== nameKey)
        .map((f) => f.key);
      const hasAnyRequired = requiredKeys.some((k) => !!accumulated[k]);
      if (hasName && hasAnyRequired) {
        const defVal = field.default_value === "Belum pasti" ? "Belum Dikonfirmasi" : field.default_value;
        accumulated[field.key] = defVal;
      }
    }
  }
  ...
  const selectValues = fields
    .filter(
      (f) =>
        (f.type === "keyword" || f.type === "select") && accumulated[f.key],
    )
